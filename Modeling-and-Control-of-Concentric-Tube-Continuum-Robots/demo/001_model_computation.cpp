...
int main(int, char**){
  ...
  computationOptions opt = {.isExternalLoads = true, // Request the model to take the Fext
                            .isComputeJacobian = false, // Disable Jacobian matrix calculation (to save calculation time)
                            .isComputeCompliance = false, // Same thing
                            .nbThreads = 4}; // Save computing power
...
// ---- EXTRACTION POUR CRVisToolkit ----
std::ofstream dataFile("robot_backbone.txt");
if (dataFile.is_open()) {
    auto yTot = ctr.GetYTot();
    dataFile << yTot << "\n";
    dataFile.close();
    std::cout << "--> Fichier 'robot_backbone.txt' genere avec succes pour Python !" << std::endl;
}
return 0;
}
