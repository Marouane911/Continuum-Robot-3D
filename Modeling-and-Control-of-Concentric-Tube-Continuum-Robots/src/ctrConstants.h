...
namespace CtrLib{
    // Debug
    #define ENABLE_DEBUG_MSG true // Set to false to disable debug messages

    #if (ENABLE_DEBUG_MSG)
        #define PRINT_DEBUG_MSG(x) std::cout << x << std::endl
    #else
        #define PRINT_DEBUG_MSG(x)
    ...
    constexpr int NB_INTEGRATION_NODES = 30;               // number of integration nodes per segment (Increasing this value, e.g to 5, yield smoother shaper in the 3d plot, but at the cost of slower computation)
