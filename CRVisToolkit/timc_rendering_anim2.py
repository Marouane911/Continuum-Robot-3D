...
for step_idx, step_str in enumerate(steps_data):
    ...
    for i in range(num_nodes):
        ...
        # Real rotation R from Quentin (lignes 3 à 11)
        R_elements = matrix_data[3:12, i] 
        M[0:3, 0:3] = R_elements.reshape((3, 3), order='F')
        
        g[i, :] = M.flatten(order='F') # Position + Rotation (in a table of size 16)
    ...
    ax_robot.set_xlim3d([-0.05, 0.05])
    ax_robot.set_ylim3d([-0.05, 0.05])
